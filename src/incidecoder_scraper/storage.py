...
class DataStore:
    ...
    def _init_schema(self) -> None:
        if self.backend == "duckdb":
            ...
            cur.close()
            self.conn.commit()
        self._ensure_image_path_column()

    def _ensure_image_path_column(self) -> None:
        if self.backend == "duckdb":
            self.conn.execute("ALTER TABLE products ADD COLUMN IF NOT EXISTS image_path TEXT")
        else:
        ...
        if self.backend == "duckdb":
            ...
            self.conn.execute(
                ...
                INSERT INTO products (
                    url, name, brand, description, image_url, image_path,
    ...
    def iter_products(self) -> Iterable[Product]:
        cur = self.conn.execute(
            "SELECT url, name, brand, description, image_url, image_path, rating_value, rating_count, categories, json_ld FROM products"
        )
        rows = cur.fetchall()
        cur.close()
        for row in rows:
